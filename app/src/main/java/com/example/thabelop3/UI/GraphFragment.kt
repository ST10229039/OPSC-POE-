...
import com.example.thabelop3.RoomDb.ExpenseWithCategory
import com.example.thabelop3.databinding.FragmentGraphBinding
...
class GraphFragment : Fragment() {
            ...
            expenseViewModel.getExpensesWithCategoryByDateRange(userId, startDate, endDate)
                .observe(viewLifecycleOwner) { expenses ->
                    ...
                    val monthlyData = expenses.groupBy {
                        ...
                        BarEntry(
                            month.toFloat(),
                            monthExpenses.sumOf { it.expense.amount }.toFloat()
                        )
                    }
                    ...
                    val pieEntries = categories.map { (categoryId, categoryExpenses) ->
                        ...
                        PieEntry(
                            value,
                            categoryExpenses.first().categoryName ?: "Category $categoryId"
                        )
                    }
    ...
    private fun updateCategoryBreakdown(expenses: List<ExpenseWithCategory>) {
        ...
        categories.forEach { (_, categoryExpenses) ->
            ...
            val color =
                ColorTemplate.MATERIAL_COLORS[categoryExpenses.first().expense.categoryId % ColorTemplate.MATERIAL_COLORS.size]
